...
/**
 * An {@link InjectionPoint} for a {@link Method}.
 *
 * @author brian.oliver
 * @since Oct-2024
 */
public class MethodInjectionPoint
    ...
    /**
     * The {@link JDKTypeDescriptor} in which the {@link MethodInjectionPoint} is defined.
     */
    private final JDKTypeDescriptor typeDescriptor;

    /**
     * The {@link MethodDescriptor} defining the {@link Method} that may be injected.
     */
    private final MethodDescriptor methodDescriptor;

    /**
     * The {@link Dependency}s defined by the {@link MethodDescriptor} for injection.
     */
    private final ArrayList<Dependency> dependencies;

    /**
     * Constructs a {@link MethodInjectionPoint}.
     *
     ...
     */
    private MethodInjectionPoint(final JDKTypeDescriptor typeDescriptor,
        ...
        final var formalParams = methodDescriptor.formalParameters()
    ...
    public <T> T inject(final Object target, final Object[] actualParameters) {
        ...
        final var method = this.methodDescriptor.getTrait(MethodType.class)
            .map(MethodType::method)
            .orElseThrow(() -> new IllegalArgumentException("MethodType is not defined for " + this.methodDescriptor));

        if (method.trySetAccessible()) {
            try {
                return (T) method.invoke(target, actualParameters);
            }
            ...
            throw new InjectionFailedException(this, "Can't inject into " + method + " as it's inaccessible");
        }
    }

    /**
     * Creates a {@link MethodInjectionPoint}.
     *
     * @param typeDescriptor               the {@link JDKTypeDescriptor}
     * @param methodDescriptor             the {@link MethodDescriptor}
     * @param qualifierAnnotationExtractor a {@link Function} to extract {@link Qualifier} {@link AnnotationTypeUsage}s
     * @return a new {@link MethodInjectionPoint}
     */
    public static MethodInjectionPoint of(final JDKTypeDescriptor typeDescriptor,
                                          final MethodDescriptor methodDescriptor,
                                          final Function<? super Traitable, Stream<AnnotationTypeUsage>> qualifierAnnotationExtractor) {

        return new MethodInjectionPoint(typeDescriptor, methodDescriptor, qualifierAnnotationExtractor);
    }
}
