...
/**
 * A {@link TokenParser} that defines an expression Binary, which is matched based on a {@link String} template.
 *
 * @author tim.berston
 * @since Nov-2024
 */
public class BinaryTokenParser
     ...
     */
    public BinaryTokenParser(final String template, final int priority, final Function<Token, NodeResolver> runnable) {
        super(template, priority, runnable);
    }
    ...
    public boolean isAllowedAfter(final List<Token> tokens) {
        ...
        return !(lastToken.tokenParser() instanceof BinaryTokenParser);
    }
}
