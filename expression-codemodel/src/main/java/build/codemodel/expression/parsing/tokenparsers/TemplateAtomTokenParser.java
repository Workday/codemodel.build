...
/**
 * A {@link TokenParser} that defines an expression Atom for use in Template expressions.
 * It is matched based on a regular expression pattern.
 *
 * @author tim.berston
 * @since Mar-2025
 */
public class TemplateAtomTokenParser
     ...
     */
    public TemplateAtomTokenParser(final String pattern, final int priority, final Function<Token, NodeResolver> runnable) {
        super(Pattern.compile(pattern), priority, runnable);
    }

    /**
     * Determines if the token is allowed after the preceding list of tokens. Always true for Template expressions.
     *
